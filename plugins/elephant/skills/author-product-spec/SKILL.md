...
0. **Resolve the output location** before writing anything: default `docs/<product>/v0/`, confirm the `<product>` slug + version with the user, and **state the final spec-dir path back**. (Downstream `decompose-roadmap` scans `docs/**` for the spec system — defaulting under `docs/` keeps that handoff reliable.)
